...
object YogaCalculator {
    ...
    private fun calculateSpecialYogas(chart: VedicChart): List<Yoga> {
        ...
        if (sunPos != null && mercuryPos != null && areConjunct(sunPos, mercuryPos)) {
            // Check if Mercury is not combust (more than 14° from Sun)
            val distance = abs(sunPos.longitude - mercuryPos.longitude)
            val isCombust = distance < 14.0 || distance > 346.0

            val strength = if (isCombust) 40.0 else calculateYogaStrength(chart, listOf(sunPos, mercuryPos))
            yogas.add(
                Yoga(
    ...
    private fun areConjunct(pos1: PlanetPosition, pos2: PlanetPosition): Boolean {
        return pos1.sign == pos2.sign
    }
    ...
    private fun isExalted(pos: PlanetPosition): Boolean {
        return when (pos.planet) {
            ...
            Planet.VENUS -> pos.sign == ZodiacSign.PISCES
